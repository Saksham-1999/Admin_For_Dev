...
export const getLastLicenseId = async () => {
  const token = sessionStorage.getItem("token");
  const response = await fetch(`${API_BASE_URL}/licenses/last-id/`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Token ${token}`,
...
export const createMultipleLicenses = async (licenseData) => {
  const token = sessionStorage.getItem("token");
  const response = await fetch(
    `${API_BASE_URL}/licenses/create-multiple-license/`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        number_of_licenses: parseInt(licenseData.numberOfLicenses),
        organisation: licenseData.organisation,
        valid_from: `${licenseData.validFrom} ${licenseData.validFromTime}:00`,
        valid_till: `${licenseData.validTill} ${licenseData.validTillTime}:00`,
      }),
    }
  );
...
export const fetchPhishingMailEml = async (msgId, recieversEmail) => {
  const token = sessionStorage.getItem("token");
  const response = await fetch(
    `${API_BASE_URL}/emaildetails/eml-datails/?msg_id=${msgId}&recievers_email=${recieversEmail}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
export const updateDisputeStatus = async (dispute_id, status) => {
  const token = sessionStorage.getItem("token");
  const response = await fetch(
    `${API_BASE_URL}/dispute/${dispute_id}/update/`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
...
export const addDisputeComment = async (dispute_id, adminComment) => {
  const token = sessionStorage.getItem("token");
  const response = await fetch(
    `${API_BASE_URL}/dispute/${dispute_id}/comments/`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export const fetchQueryDetails = async () => {
  const token = sessionStorage.getItem("token");
...
export const submitQueryResponse = async (queryId, adminResponse) => {
  const token = sessionStorage.getItem("token");
  const response = await fetch(`${API_BASE_URL}/queries/${queryId}/respond/`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Token ${token}`,
